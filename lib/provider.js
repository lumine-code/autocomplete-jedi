...
module.exports = {
  ...
  inclusionPriority: 2,
  suggestionPriority: atom.config.get("autocomplete-jedi.priority"),
  ...
  noExecutableError(error) {
    ...
    atom.notifications.addWarning("autocomplete-jedi unable to find python binary.", {
      detail: `Please set the Python command in package settings.
Detailed error message: ${error}

Current config: ${atom.config.get("autocomplete-jedi.pythonCommand")}`,
      dismissable: true,
    });
    ...
    this.provider = new this.BufferedProcess({
      ...
      stderr: (data) => {
        ...
        if (data.indexOf("jedi") > -1) {
          if (atom.config.get("autocomplete-jedi.showErrors")) {
...
to hide such errors in future. Traceback output:`,
          ...
          atom.notifications.addError("autocomplete-jedi traceback output:", {
            detail: `${data}`,
            dismissable: true,
          });
        }
  ...
  constructor() {
    ...
    this.disposables.add(
      atom.config.observe("autocomplete-jedi.pythonCommand", (value) => {
      ...
      atom.config.observe("autocomplete-jedi.enableCompletion", (value) => {
    ...
    try {
      this.triggerCompletionRegex = RegExp(atom.config.get("autocomplete-jedi.triggerRegex"));
    } catch (err) {
      atom.notifications.addWarning(
...
Falling back to default value.`,
      ...
      atom.config.set("autocomplete-jedi.triggerRegex", "([\\.\\ (]|[a-zA-Z_][a-zA-Z0-9_]*)");
      this.triggerCompletionRegex = /([. (]|[a-zA-Z_][a-zA-Z0-9_]*)/;
    }

    const selector = "atom-text-editor[data-grammar~=python]";
    this.disposables.add(
      atom.commands.add(selector, "autocomplete-jedi:go-to-definition", () => {
        this.goToDefinition();
      }),
    );
    this.disposables.add(
      atom.commands.add(selector, "autocomplete-jedi:show-usages", () => {
    ...
    this.disposables.add(
      atom.commands.add(selector, "autocomplete-jedi:override-method", () => {
        const editor = atom.workspace.getActiveTextEditor();
        const bufferPosition = editor.getCursorBufferPosition();
    ...
    this.disposables.add(
      atom.commands.add(selector, "autocomplete-jedi:rename", () => {
        const editor = atom.workspace.getActiveTextEditor();
        const bufferPosition = editor.getCursorBufferPosition();
        this.getUsages(editor, bufferPosition).then((usages) => {
          if (this.renameView) {
    ...
    this.disposables.add(
      atom.commands.add("atom-workspace", "autocomplete-jedi:add-roots-to-extra-paths", () => {
        ...
        atom.config.set("autocomplete-jedi.extraPaths", current.join(";"));
        if (added > 0) {
  ...
  generateRequestConfig() {
    const extraPaths = this.InterpreterLookup.applySubstitutions(
      atom.config.get("autocomplete-jedi.extraPaths").split(";"),
    );
    return {
      extraPaths: extraPaths,
      caseInsensitive: atom.config.get("autocomplete-jedi.caseInsensitive"),
      showDocStrings: atom.config.get("autocomplete-jedi.showDocStrings"),
      fuzzyMatching: atom.config.get("autocomplete-jedi.fuzzyMatching"),
    };
  },
  ...
  fuzzyFilter(candidates, query) {
    ...
    const matcher = atom.tools.fuzzyMatcher.setCandidates(candidates.map((c) => c.text));
    return matcher
  ...
  getSuggestions({ editor, bufferPosition, prefix }) {
    ...
    if (atom.config.get("autocomplete-jedi.fuzzyMatching")) {
